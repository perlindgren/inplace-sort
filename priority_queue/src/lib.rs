...
mod tests {
    ...
    unsafe fn assert_next<T: PartialOrd, const N: usize>(
    ...
    // SAFETY: must be run inside a critical section
    unsafe fn assert_tail<T: PartialOrd, const N: usize>(pq: &PriorityQueue<T, N>, idx: NodePtr) {
        unsafe {
            assert_eq!(*pq.tail_ptr.get(), Some(idx));
    ...
    fn new() {
        ...
        critical_section::with(|_| unsafe {
            assert_eq!(*pq.head_ptr.get(), None);
            assert_eq!(*pq.free_ptr.get(), Some(0));
        });
    ...
    fn pop_length_one_list() {
        ...
        critical_section::with(|cs| unsafe {
            assert_eq!(pq.get_head_ptr(), Some(0));
            assert_eq!(pq.get_tail_ptr(), Some(0));
            assert_eq!(pq.get_min_ptr(), Some(0));
            assert_eq!(pq.get_free_ptr(), Some(1));
        ...
        critical_section::with(|_| unsafe {
            assert_eq!(pq.get_head_ptr(), None);
            assert_eq!(pq.get_tail_ptr(), None);
            assert_eq!(pq.get_min_ptr(), None);
            assert_eq!(pq.get_free_ptr(), Some(0));
        });
    }
    ...
    fn pop_length_two_list_ordered() {
        ...
        critical_section::with(|_| unsafe {
            assert_eq!(pq.get_head_ptr(), Some(0));
            assert_eq!(pq.get_tail_ptr(), Some(1));
            assert_eq!(pq.get_min_ptr(), Some(0));
            assert_eq!(pq.get_free_ptr(), Some(2));
        });
        ...
        critical_section::with(|_| unsafe {
            assert_eq!(pq.get_head_ptr(), Some(1));
            assert_eq!(pq.get_min_ptr(), Some(1));
            assert_tail(&pq, 1);
    ...
    fn pop_end() {
        ...
        critical_section::with(|cs| unsafe {
            ...
            assert_tail(&pq, 3);
            // Verify edges of free list
            assert_eq!(pq.get_free_ptr(), Some(4));
            assert_next(&pq, 4, None);
    ...
    #[test]
    fn pop_middle() {
        let pq = PriorityQueue::<i32, 7>::new();
        ...
        critical_section::with(|_| unsafe {
            assert_eq!(pq.get_head_ptr(), Some(0));
            assert_eq!(pq.get_min_ptr(), Some(2));
            assert_tail(&pq, 6);
            // Verify edges of free list
            assert_eq!(pq.get_free_ptr(), Some(5));
        ...
        critical_section::with(|_| unsafe {
            ...
            assert_tail(&pq, 4);
            // Verify edges of free list
            assert_eq!(pq.get_free_ptr(), Some(6));
            assert_next(&pq, 5, None);
        });
