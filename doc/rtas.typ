...
#show: ieee.with(
  ...
  abstract: [
    ...
    for preemption points, allowing us to give a $O(1)$ upper bound on the inferred blocking
    sections. The overall complexity is $O(n)$ for `insert` and $O(1)$ for `new`, `peek` and `pop`,
    as expected for our insertion sort implementation. The queue is backed by a fixed-size array,
    and can be allocated either statically, on the heap or on the stack, in compliance to the Rust
    ownership model. Potential applications include real-time scheduling, event management, and
    graph algorithms witch requirements to predictable blocking times and memory safety.
  ],
  authors: (
    (
...
= Introduction
The Rust language, with its strong emphasis on memory safety and concurrency, offers a promising
platform for implementing safety, security, and timing critical systems. Priority queues find many
applications in software systems, including real-time scheduling, event management, and graph
algorithms. In this paper we will explore opportunities and challenges involved towards a
thread-safe, in-place priority queue implementation. To provide upper bounds on blocking times in
concurrent settings, we extend Rust's critical section abstraction to support preemption points,
allowing us to provide constant time $O(1)$ upper bounds on blocking times in concurrent settings.
...
mode, excluding dependencies to the standard library and any underlying operating system. This makes
...
In this paper, we sketch a thread-safe priority queue implementation, where there may exist multiple
threads inserting and removing items concurrently, with bounded blocking times.
...
For the purpose of Earliest Deadline First (EDF) scheduling, we seek a priority queue implementation
...
3. If the queue is neither full nor empty, we need to find the correct position for the new value
  ...
  is illegal in Rust, as it implies undefined behavior (UB). However, following the `head`, always
  lead us to an initialized value (4 in our example). We start by introducing a local cursor
  variable, initialized to the `head` of the queue, and we read the value at the cursor.
